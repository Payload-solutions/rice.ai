...
def recomendation(conv_value_nitrogen, conv_value_loss):
    ...
    return f"La planta se encuentra con una coloración aceptable con una presencia de nitrogeno de {float(conv_value_nitrogen):.2f}" if float(
        conv_value_nitrogen) >= 85.00 else f"Coloración amarilla con una presencia de nitrogeno de {float(conv_value_loss):.2f}; considerar incluir más nitrógeno en el cultivo"
